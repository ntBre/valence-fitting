...
class BaseFragment(object):
    ...
    def BaseFragmentFromRdmol(cls, rdmol):
        ...
        """
        baseAtoms = [
            BaseAtom(atom.GetSymbol(), atom.GetIdx())
        ...
        for ring in rings:
            ...
            for r in ring:
                for nei in G.neighbors(r):
                    if nei not in ring:
                        if G.get_edge_data(r, nei)[
                            "bondtype"
                        ] != Chem.rdchem.BondType.SINGLE or (
                            G.degree(nei) == 1
    ...
    def getAllSingleChains(self, rings):
        ...
        for g in chain_graphs:
            chain_type_set = set(
                [g.node[idx]["atom"].chain_num for idx in g.nodes()]
            )
    ...
    def cutChain(self, chain):
        ...
        if len(chain.nodes()) == 4:
            ...
            one_path = nx.shortest_path(
                chain, terminal_atom[0], terminal_atom[1]
            )
        ...
        index_score_lt1_dict = {
            node: chain.node[node]["atom"].score
            for node in chain.nodes()
            if chain.node[node]["atom"].score < 2
        }
        index_score_list = sorted(
            index_score_dict.items(), key=lambda e: e[1], reverse=True
        )
        ...
        for idx in index_score_lt1_dict:
            if degree[idx] == 2:
                ...
                if (
                    chain.get_edge_data(idx, neigs[0])["bondtype"] == 1
                    and chain.get_edge_data(idx, neigs[1])["bondtype"] == -1
                ):
        ...
        for idx in index_score_lt1_dict:
            ...
            if len(neis) == 2:
                per, bak = (
                    chain.node[neis[0]]["atom"].score,
                    chain.node[neis[1]]["atom"].score,
                )
                if per > 1 and bak > 1:
                    if per > bak:
            ...
            if chain.get_edge_data(tm, end_idx)["bondtype"] == 0:
                ...
                if chain.node[end_idx]["atom"].score == 0:
                    for eend_idx in chain.neighbors(end_idx):
                        if eend_idx != tm:
                            chain.get_edge_data(end_idx, eend_idx)[
                                "bondtype"
                            ] = 0
        # 3f.
        for k in index_score_dict:
            ...
            bond_flag = [
                chain.get_edge_data(k, nei)["bondtype"] == 0 for nei in neis
            ]
            if all(bond_flag):
                topscore = -1
                topscore_idx = 0
        ...
        for grp in groups:
            ...
            one_path = nx.shortest_path(
                tmp_subg, terminal_atom[0], terminal_atom[1]
            )
            if chain_len == 4:
        ...
        for node in chain.nodes():
            ...
            if len(neigs) != 2:
                continue
            if (
                chain.get_edge_data(node, neigs[0])["bondtype"] == 0
                and chain.get_edge_data(node, neigs[1])["bondtype"] == 0
            ):
                for nei in neigs:
