...
filters := $(CURATE)/charge-filter.py $(CURATE)/filters.py

## step 2b filter sage data sets for charge issues
$(CURATE)/sage/filtered-opt.json: $(CURATE)/sage/opt.json $(filters)
	python $(CURATE)/charge-filter.py --input $< --output $@
$(CURATE)/sage/filtered-td.json: $(CURATE)/sage/td.json $(filters)
	python $(CURATE)/charge-filter.py --input $< --output $@

## step 2c filter the new datasets for charge issues
$(CURATE)/datasets/filtered-opt.json: $(CURATE)/datasets/core-opt.json $(filters)
	python $(CURATE)/charge-filter.py --input $< --output $@
$(CURATE)/datasets/filtered-td.json: $(CURATE)/datasets/core-td.json $(filters)
	python $(CURATE)/charge-filter.py --input $< --output $@
